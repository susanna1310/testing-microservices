...
public class RouteControllerTest
{
    ...
    public void testCreateAndModifyRoute() throws Exception
    {
        ...
        Mockito.when(routeService.createAndModify(Mockito.any(RouteInfo.class), Mockito.any(HttpHeaders.class)))
            .thenReturn(response);
        String requestJson = JSONObject.toJSONString(createAndModifyRouteInfo);
        String result = mockMvc.perform(
                MockMvcRequestBuilders.post("/api/v1/routeservice/routes").contentType(MediaType.APPLICATION_JSON)
                    .content(requestJson))
            .andExpect(MockMvcResultMatchers.status().isOk())
    ...
    {
        Mockito.when(routeService.getRouteByStartAndTerminal(Mockito.anyString(), Mockito.anyString(),
