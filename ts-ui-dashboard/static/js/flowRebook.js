        ...
        success: function (result) {
            ...
            for (var i = 0; i < size; i++) {
                ...
                $("#my_orders_result").append(
                    ...
                    "<label class='control-label noshow_component my_order_list_from_name'>" + order["from"] +
                    "</label>" +
                    "</div>" +
                    "</div>" +
                    "<div class='div form-group'>" +
                    ...
                    "<label class='order_id control-label noshow_component my_order_list_accountId' >" +
                    order["accountId"] + "</label>" +
                    addCancelAandRebookButtonOrNot(order) +
                    // "<button type='button' class='ticket_cancel_btn btn btn-primary'>" + "Cancel Order" +
                    // "</button>" + "<button type='button' class='order_rebook_btn btn btn-primary'>" + "Change your
                    // railway ticket" + "</button>" +
                    "</div>" +
                    "</div>" +
                    "</form>" +
                    "</div>" +
...
function addCancelAandRebookButtonOrNot(order)
{
    var str = "";
    if (order["status"] == 1) {
        str += "<button type='button' class='order_rebook_btn btn btn-primary'>" + "Change your railway ticket" +
            "</button>";
    }
...
function addListenerToOrderChange()
{
    ...
    for (var i = 0; i < ticketChangeButtonSet.length; i++) {
        ticketChangeButtonSet[i].onclick = function () {
            ...
            $("#ticket_rebook_confirm_old_tripId").text(
                $(this).parents("form").find(".my_order_list_train_number").text());
        }
    }
}
            ...
            if (result[0] != null) {
                ...
                for (var i = 0, l = obj.length; i < l; i++) {
                    $("#tickets_change_list_table").find("tbody").append(
                        ...
                        "</select>" +
                        "</td>" +
                        "<td>" + "<button class='btn btn-primary ticket_rebook_button'>" + "Rebook" + "</button>" +
                        "</td>" +
                        "</tr>"
                    );
                }
