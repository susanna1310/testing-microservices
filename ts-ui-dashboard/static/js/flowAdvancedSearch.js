            ...
            if (result.status = true) {
                ...
                for (var i = 0, l = obj.length; i < l; i++) {
                    $("#flow_advance_reserve_booking_list_table").find("tbody").append(
                        ...
                        "</select>" +
                        "</td>" +
                        "<td class='booking_seat_price_confort noshow_component'>" + obj[i]["priceForFirstClassSeat"] +
                        "</td>" +
                        "<td class='booking_seat_price_economy noshow_component'>" + obj[i]["priceForSecondClassSeat"] +
                        "</td>" +
                        "<td>" + "<button class='btn btn-primary advance_ticket_booking_button'>" + "Booking" +
                        "</button>" + "</td>" +
                        "</tr>"
                    );
                }
...
function flow_advance_addListenerToBookingTable()
{
    ...
    for (var i = 0; i < ticketBookingButtonSet.length; i++) {
        ticketBookingButtonSet[i].onclick = function () {
            ...
            if (seatType == 2) {
                $("#flow_advance_reserve_ticket_confirm_price").text(
                    $(this).parents("tr").find(".booking_seat_price_confort").text());
            } else if (seatType == 3) {
                $("#flow_advance_reserve_ticket_confirm_price").text(
                    $(this).parents("tr").find(".booking_seat_price_economy").text());
            }
        ...
        success: function (result) {
            ...
            for (var i = 0, l = obj.length; i < l; i++) {
                $("#flow_advance_reserve_contacts_booking_list_table").find("tbody").append(
                    "<tr>" +
                    "<td>" + i + "</td>" +
                    "<td class='advance_search_booking_contacts_contactsId' style='display:none'>" + obj[i]["id"] +
                    "</td>" +
                    "<td class='advance_search_booking_contacts_name'>" + obj[i]["name"] + "</td>" +
                    "<td class='advance_search_booking_contacts_documentType'>" +
                    convertNumberToDocumentType(obj[i]["documentType"]) + "</td>" +
                    ...
                    "<td>" + "<label><input class='advance_search_booking_contacts_select' type='radio' />" + "Select" +
                    "</label>" + "</td>" +
                    "</tr>"
                );
            }
            $("#flow_advance_reserve_contacts_booking_list_table").find("tbody").append(
                "<tr>" +
                "<td>" + obj.length + "</td>" +
                "<td class='booking_contacts_name'>" + "<input id='advance_search__booking_new_contacts_name'>" +
                "</td>" +
                "<td>" +
                ...
                "</select>" +
                "</td>" +
                "<td class='advance_search_booking_contacts_documentNumber'>" +
                "<input id='flow_advance_booking_new_contacts_documentNum'>" + "</td>" +
                "<td class='advance_search_booking_contacts_phoneNumber'>" +
                "<input id='flow_advance_booking_new_contacts_phoneNum'>" + "</td>" +
                "<td>" +
                "<label><input id='flow_advance_booking_new_contacts_select' class='flow_advance_booking_contacts_select' name='booking_contacts' type='radio' />" +
                "Select" + "</label>" + "</td>" +
                "</tr>"
            );
        },
...
$("#flow_advance_reserve_ticket_select_contacts_confirm_btn").click(function () {
    ...
    if ($('#flow_advance_reserve_need_food_or_not').is(":checked")) {
        ...
        if ($('#flow_advance_reserve_preserve_food_type').find("option:selected").val() == 1) {
            ...
            $('#flow_advance_reserve_ticket_confirm_food_station').text(
                $('#food-station-list').find("option:selected").text());
            $('#flow_advance_reserve_ticket_confirm_food_store').text(
                $('#food-stores-list').find("option:selected").text());
        }
    ...
    if ($('#flow_advance_reserve_need_consign_or_not').is(":checked")) {
        ...
        $('#flow_advance_reserve_ticket_confirm_consignee_name').text(
            $(" #flow_advance_reserve_name_of_consignee ").val());
        $('#flow_advance_reserve_ticket_confirm_consignee_phone').text(
            $(" #flow_advance_reserve_phone_of_consignee").val());
        $('#flow_advance_reserve_ticket_confirm_consign_weight').text(
            $(" #flow_advance_reserve_weight_of_consign ").val());
    } else {
...
function flow_advance_preserveCreateNewContacts()
{
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $("#flow_advance_reserve_ticket_confirm_documentType").text(
                convertNumberToDocumentType(result["contacts"]["documentType"]));
...
$("#flow_advance_reserve_ticket_confirm_confirm_btn").click(function () {
    ...
    if (null != $('#flow_advance_reserve_ticket_confirm_food_type').text() && "" !=
        $('#flow_advance_reserve_ticket_confirm_food_type').text())
    {
        if ($('#flow_advance_reserve_ticket_confirm_food_type').text() == "Train Food") {
            ...
            orderTicketInfo.foodPrice =
                parseFloat($('#flow_advance_reserve_ticket_confirm_food_price').text().substr(1));
            ...
            orderTicketInfo.foodName = $('#flow_advance_reserve_ticket_confirm_food_name').text();
            orderTicketInfo.foodPrice =
                parseFloat($('#low_advance_reserve_ticket_confirm_food_price').text().substr(1));
        } else {
