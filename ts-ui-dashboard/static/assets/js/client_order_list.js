var appConsign = new Vue({
    ...
    data: {
        myOrderList: [],
        tempOrderList: [],
        statusExpress: ["Not Paid", "Paid & Not Collected", "Collected", "Cancel & Rebook", "Cancel", "Refunded",
            "Used", "other"],
        orderId: '',
        tripId: '',
        newTripId: '',
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    for (var i = 0; i < size; i++) {
                        ...
                        that.tempOrderList[i].boughtDate =
                            that.convertNumberToDateTimeString(that.tempOrderList[i].boughtDate)
                    }
                    ...
                    $.ajax({
                        ...
                        success: function (result) {
                            if (result["status"] == 1) {
                                ...
                                if (result.data['differenceMoney'] != null || result.data['differenceMoney'] !=
                                    'null')
                                {
                                    $('#my-prompt2').modal({
                                        relatedTarget: this,
                ...
                success: function (result) {
                    if (result["status"] == 1) {
                        ...
                        for (var i = 0, l = obj.length; i < l; i++) {
                            ...
                            that.tempSearchRoutes[i].startingTime =
                                that.convertNumberToTimeString(that.tempSearchRoutes[i].startingTime);
                            that.tempSearchRoutes[i].endTime =
                                that.convertNumberToTimeString(that.tempSearchRoutes[i].endTime);
                        }
                        that.searchRoutes = that.searchRoutes.concat(that.tempSearchRoutes);
                        that.initSeatClaass(that.searchRoutes.length);
                    }
