var adSearch = new Vue({
    ...
    methods: {
        ...
        adSearchPath()
        {
            ...
            if (advanceSearchInfo.departureTime == null || this.checkDateFormat(advanceSearchInfo.departureTime) ==
                false)
            {
                ...
                success: function (result) {
                    if (result.status = 1) {
                        ...
                        for (var i = 0, l = obj.length; i < l; i++) {
                            that.adTicketSearchResult[i].startingTime =
                                that.flow_advance_convertNumberToTimeString(obj[i]["startingTime"]);
                            that.adTicketSearchResult[i].endTime =
                                that.flow_advance_convertNumberToTimeString(obj[i]["endTime"]);
                        }
                    }
                }, error: function (e) {
