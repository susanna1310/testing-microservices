...
import rebook.entity.Trip;
import rebook.entity.TripAllDetail;
import rebook.entity.TripResponse;
 ...
 * As such we need to test the equivalence classes for the attributes of the RebookInfo object. Because the service
 * communicates with other services via RestTemplate,we use MockRestServiceServer to mock the responses of the external services.
 */
@SpringBootTest
...
public class PostRebookDifferenceTest
{
	...
	*/
    ...
    /*
     * Similar to the test above, this equivalence class also exists for the orderId, which does not cause a separate
     * response. This will in most cases cause an exception, if the returned object is null or not fully initialized.
     * As such the request will be interrupted and return a null response.
     */
    @Test
     ...
     * we can achieve this hypothetical, if the old trainType is the more expensive high speed train with the tripId beginning with
     * "G" or "D". The response depends on the inside_pay_service, but from the specifications we can see that it does
     * not fail in that case. That means the returned response will be successful.
     */
    @Test
    void validTestNegativeDifference() throws Exception {
        ...
        order.setTo("1");
        order.setPrice("200");
        order.setTrainNumber("G");
        Response<rebook.entity.Order> mockResponse1 = new Response<>(1, "Success", order);
        URI uri = UriComponentsBuilder.fromUriString("http://ts-order-service:12031/api/v1/orderservice/order//1").build().toUri();
        mockServer.expect(ExpectedCount.once(), requestTo(uri))
                .andExpect(method(HttpMethod.GET))
                .andRespond(withSuccess(objectMapper.writeValueAsString(mockResponse1), MediaType.APPLICATION_JSON));
        ...
        Response mockResponse6 = new Response<>(1, "Success", "not relevant");
        uri = UriComponentsBuilder.fromUriString("http://ts-order-service:12031/api/v1/orderservice/order/" + id).build().toUri();
        mockServer.expect(ExpectedCount.once(), requestTo(uri))
                .andExpect(method(HttpMethod.POST))
                .andRespond(withSuccess(objectMapper.writeValueAsString(mockResponse6), MediaType.APPLICATION_JSON));
        ...
        //Actual request to the endpoint we want to test
        String requestJson = "{\"loginId\":\"1\", \"orderId\":\"1\", \"oldTripId\":\"G\", \"tripId\":\"Z\", \"seatType\":2, \"date\":null}";
        ...
        assertEquals("1", order.getFrom());
        assertEquals(id, order.getId());
        assertEquals("Z", order.getTrainNumber());
        assertEquals("1", order.getTo());
        assertEquals(3, order.getStatus());
        assertEquals(new Response<>(1, "Success", JSONObject.parseObject(result, Response.class).getData()), JSONObject.parseObject(result, Response.class));
