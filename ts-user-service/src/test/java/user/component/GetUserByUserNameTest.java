...
/*
 * This endpoint GETs the User objects from the user repository with the given name. As this endpoint has no body or argument, there is
...
public class GetUserByUserNameTest {
	...
	*/
     ...
     * As such the combination of a valid input name and the insertion before, we reach the corresponding response. As the
     * name is of type String every String is a valid name.
     */
    @Test
    void validTestGetObject() throws Exception {
        ...
        String result = mockMvc.perform(get("/api/v1/userservice/users/{userName}", "name")
                )
    ...
    void invalidTestMissingBody() {
        assertThrows(IllegalArgumentException.class, () -> {mockMvc.perform(get("/api/v1/userservice/users/{userName}")
        );});
	...
	*/
    ...
    /*
     * For this equivalence class we give an malformed name with invalid characters as the URL parameter, which should
     * cause a 4xx client error.
     */
    @Test
    void invalidTestNonCorrectFormatName() throws Exception {
        mockMvc.perform(get("/api/v1/userservice/users/{userName}", "a/b")
                )
