...
/*
 * In this endpoint we do a POST request of a station id list and get the corresponding name list. It takes a List<String> as body.
...
@Testcontainers
public class PostStationsNameListTest {
	...
	*/
     ...
     * As such the combination of a valid input id from the equivalence class and the insertion before, we reach the
    ...
    void validTestCorrectObject() throws Exception {
        ...
        String result = mockMvc.perform(post("/api/v1/stationservice/stations/namelist")
	...
	*/
     ...
     */
    @Test
    void bodyVarIdListInvalidTestStringIsDuplicate() {
        List<String> nameList = new ArrayList<>();
        nameList.add(null);
        ...
        assertThrows(NestedServletException.class, () -> {mockMvc.perform(post("/api/v1/stationservice/stations/namelist")
                .contentType(MediaType.APPLICATION_JSON)
                .content(requestJson)
        );});
    }
    ...
    /*
     * For this equivalence class, we have a mixed list as input with only a few existing ids in the repository.
     * As such the returned data should be a list of only a few names.
     */
    @Test
    void bodyVarIdListValidTestMixedList() throws Exception {
        List<String> nameList = new ArrayList<>();
        for (int i = 0; i < 10; i++) {
            nameList.add(String.valueOf(i));
            if(i % 2 == 0) {
        ...
        for (String station : stations) {
            assertEquals(String.valueOf(j), station);
            j += 2;
        }
        assertEquals(new Response<>(1, "Success", stations), JSONObject.parseObject(result, Response.class));
    ...
    /*
     * For the last equivalence class from inputs we want to get the outcome response that there is no content. This can
     * be achieved with an empty input list or an input list with non-existing ids in the repository
     */
    @Test
    void bodyVarIdListValidTestEmptyList() throws Exception {
        List<String> nameList = new ArrayList<>();
        String requestJson = JSONObject.toJSONString(nameList);
        ...
        assertEquals(new Response<>(0, "No stationNamelist according to stationIdList", JSONObject.parseObject(result, Response.class).getData()), JSONObject.parseObject(result, Response.class));
